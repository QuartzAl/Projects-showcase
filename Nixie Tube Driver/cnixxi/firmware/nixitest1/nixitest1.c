...
static void HandleCommand( uint32_t dmdword )
{
	...
	// Note: To get here, DEBUG0's LSB must be 0x4x, where command is that 'x'
...
int main()
{
	...
	GPIOD->CFGLR = 
		(GPIO_Speed_10MHz | GPIO_CNF_OUT_PP)<<(4*5) | // Right comma
		(GPIO_Speed_10MHz | GPIO_CNF_OUT_PP)<<(4*6) | // Left comma
		(GPIO_Speed_10MHz | GPIO_CNF_OUT_PP)<<(4*7) | // DIG_0
	...
	GPIOC->CFGLR = 
		(GPIO_Speed_10MHz | GPIO_CNF_OUT_PP)<<(4*0) | // DIG_2
		(GPIO_Speed_10MHz | GPIO_CNF_OUT_OD_AF)<<(4*1) | // SCL
		(GPIO_Speed_10MHz | GPIO_CNF_OUT_OD_AF)<<(4*2) | // SDA
	...
	*DMDATA0 = 0;
	...
	target_feedback = 170;
	// Delay_Ms(1000);
	// ApplyOnMask( GenOnMask( 8));
	// Delay_Ms(1000);
	// ApplyOnMask( GenOnMask( 9));
	// Delay_Ms(1000);
	// for (int i = 0; i < 12; i++){
	...
	while(1)
	{
		...
		// printf( "VDD: %d FB: %d\n", lastadc>>ADC_IIR, feedback_vdd);
		// printf("Target feedback: %d\n", target_feedback);
